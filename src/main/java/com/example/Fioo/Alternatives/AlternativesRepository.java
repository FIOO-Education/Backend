...
public interface AlternativesRepository extends JpaRepository<Alternatives, Long> {
    @Query("SELECT a FROM Alternatives a LEFT JOIN Questions q ON a.codQuestion = q.codQuestion LEFT JOIN Activities ac ON q.codActivity = ac.codActivity WHERE q.codActivity = :codActivity")
